...
export default function Response({ data }: ResponseProps<DndListProps>) {
  ...
  const renderCard = useCallback((card: Variant, index: number) => {
    return <DndCard key={card.id} id={card.id} text={card.label} index={index} moveCard={moveCard} />;
  }, []);
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      <FormControl>
        <FormLabel id="radio-buttons-group-label">Перетаскивайте элементы в правильном порядке</FormLabel>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 1, marginTop: "1em" }}>
          {variants.map((option: Variant, index: number) => renderCard(option, index))}
        </Box>
      </FormControl></DndProvider>
  );
}
